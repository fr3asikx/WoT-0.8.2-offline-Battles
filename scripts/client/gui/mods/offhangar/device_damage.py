...
_MODULE_STAT_SPEC = {
    ...
    # Hull traverse is ENGINE work, not track work. A damaged (not thrown)
    # track used to be listed here and halved the traverse rate, which the
    # mod's author reports is not how the tank behaves: a damaged track
    # costs nothing on the turn, a damaged engine does. This entry only ever
    # governed the DAMAGED case anyway - a DESTROYED track blocks the turn
    # outright through the is_tracked flag in the battle tick, and that path
    # is untouched. No client data exists either way: 0.8.2 ships no
    # module-to-stat table at all (avatar.py only gates input on a destroyed
    # engine/track/gun), so both versions are reconstructions.
    'traverse': (('engineHealth',), DAMAGED_MODULE_EFFICIENCY, 0.0),
...
if __name__ == '__main__':
    ...
    check('a damaged track costs NOTHING on the turn',
          module_stat_factor(dmg5, set(), td, 'traverse') == 1.0)
    dmg6 = dict(dh)
    dmg6['engineHealth'] = 100
    check('a damaged engine halves traverse',
          abs(module_stat_factor(dmg6, set(), td, 'traverse') - 0.5) < 1e-9)
    check('a damaged engine also costs mobility',
          abs(module_stat_factor(dmg6, set(), td, 'mobility') - 0.5) < 1e-9)

...
def _read_document():
    '''Overrides from disk, falling back to the .bak the writer leaves behind.

    atomic_write_user_file() keeps the previous file as <name>.bak, so a save
    interrupted part-way can leave the live file missing or half-written while
    the previous one is intact. Without trying both, that recovery copy would
    never be read and the player would silently lose every hand-placed zone.'''
